...
<div class='body col-12'>
	...
	<div class='game col-10'>
		...
		<div id='player-equips-table'>
			...
			<?php
			$sql = "SELECT * FROM `player_items` WHERE `player_id`='".$player->getID()."'";
			$query = query($sql);
			if(mysql_num_rows($query) == 0) {
				echo"
				<tr>
					<td colspan='5'> Você não possui nenhum item. </td>
				</tr>	 
				";
			} else {
			while($row = mysql_fetch_assoc($query)) {
				...
				<tr>
					<td>
						<span style='color: ".$item->getRarityColor()."'>".$item->getName()."</span>(".$row['amount'].")
					</td> 
					<td>
						".$item->getType()."
					</td>
				...
				if($item->getType() == 2) {
					echo"  
						<td style='background-color: #00cc00; color: white; cursor: pointer'> Usar </td>
					";
				}
				echo"	
				</tr>
