...
<div class='body col-12'>
	...
	<div class='game col-10'>
		Hall <input type='button' value='Voltar' onClick='location.href="hall.php"'><br>
		<hr style='border: 1px solid black'> <br><br>
		<div class='scroll-div'>
		<table border='0' style='margin: 0 auto'>
			<tr>
				<td colspan='3' style='border-bottom: 1px solid black'> Jogadores neste andar </td>
			</tr>
		<?php
		$sql = "SELECT * FROM `users` WHERE `actual_floor`='".$player->getActualFloor()."'";
		$query = query($sql);
		while($row = mysql_fetch_assoc($query)) {
			echo"
			<tr>
				<td> ".$row['username']." </td>
				<td> <input type='button' value='Perfil' class='btn'> </td>
			";
			if($player->getID() != $row['id']) {
				$sql2 = "SELECT * FROM `player_friends` WHERE `player_id`='".$player->getID()."' AND `friend_id`='".$row['id']."'";
				$query2 = query($sql2);
				if(mysql_num_rows($query2) == 0) {
					echo" <td> <input type='button' value='Adicionar' class='btn btn-success' onClick='location.href=\"friend_request.php?i=".$row['id']."\"'> </td> ";
				}
			}
			echo"
			</tr>		
